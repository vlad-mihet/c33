...
describe('budgetForecastSchema', () => {
  ...
  it('should reject fiscal year below 2000', () => {
    ...
    expect(error?.details[0].message).toContain('must be greater than or equal to 2000');
  });
  ...
  it('should reject fiscal year above 2100', () => {
    ...
    const { error } = budgetForecastSchema.validate(invalidData);
    expect(error).toBeDefined();
    expect(error?.details[0].message).toContain('must be less than or equal to 2100');
  });
  ...
  it('should reject invalid quarter', () => {
    ...
    const { error } = budgetForecastSchema.validate(invalidData);
    expect(error).toBeDefined();
    expect(error?.details[0].message).toContain('must be less than or equal to 4');
  });
