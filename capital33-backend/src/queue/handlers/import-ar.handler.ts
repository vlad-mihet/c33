...
@Injectable()
export class ImportArHandler implements IJobHandler {
  private readonly logger = new Logger(ImportArHandler.name);

  constructor(
    @InjectModel(AccountsReceivable.name)
    private readonly model: Model<AccountsReceivableDocument>,
  ) {}
  ...
  async handle(job: JobDocument): Promise<ImportSummary> {
    ...
    this.logger.log(`Importing Accounts Receivable from: ${filePath}`);
    ...
    try {
      ...
      const config = manifest.importTypes['ar'];
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const { error, value } =
            accountsReceivableSchema.validate(normalized);
          if (error) {
            summary.failed++;
            summary.errors.push({
              row: rowNumber,
