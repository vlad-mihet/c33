...
@Injectable()
export class ImportExpenseClaimsHandler implements IJobHandler {
  private readonly logger = new Logger(ImportExpenseClaimsHandler.name);

  constructor(
    @InjectModel(ExpenseClaims.name)
    private readonly model: Model<ExpenseClaimsDocument>,
  ) {}
  ...
  async handle(job: JobDocument): Promise<ImportSummary> {
    ...
    this.logger.log(`Importing Expense Claims from: ${filePath}`);
    ...
    try {
      ...
      const config = manifest.importTypes['expenseClaims'];
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const { error, value } = expenseClaimsSchema.validate(normalized);
          if (error) {
            summary.failed++;
            summary.errors.push({
              row: rowNumber,
